/**
 * BELOW JS FUNCTIONS USEFUL FOR VARIOUS TASKS
 * --------------------------------------------------------------
...
 * getViewport() => to retrieve the dimension (width + height) of the viewport (viewport = html tag)
 * scroll_sniffer() => add a class to the body tag when the scroll is at a certain distance from the top
 * smartresize() => replaces the "resize" of jQuery, executes the code only after the magnification is stopped
 * scrollEnd() => replace the "scroll" of jQuery, execute the code only once when the scroll is stopped
 * viewportChecker() => a jQuery plugin that detects when an element enters viewport and adds a class to it.
